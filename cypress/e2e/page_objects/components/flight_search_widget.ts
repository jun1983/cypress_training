...
class FlightSearchWidget extends BaseComponent{
  clickSearchFlightButton() {
    this.root.find(`[data-testid="search-flights-btn"] button`).click()
  }

  departurePort(): AirportFieldComponent{
     return new AirportFieldComponent(this.root.find(`[data-testid="departure-port"]`))
  }

  arrivalPort(): AirportFieldComponent{
    return new AirportFieldComponent(this.root.find(`[data-testid="arrival-port"]`))
  }

  travelDate(){
    return new InputFieldComponent(this.root.find(`[data-testid="travel-dates"]`))
  }
}
